...
```text
DW_mart_build/
├── 01_create_warehouse_schema.sql # Star schema DDL
...
- **SQL Files:**
  - [`01_create_warehouse_schema.sql`](DW_mart_build/01_create_warehouse_schema.sql) – Defines star schema with 4 core tables
  - [`02_load_schema_dw.sql`](DW_mart_build/02_load_schema_dw.sql) – Extracts CSVs from GCS and loads into warehouse tables
...
- **SQL File:** [`03_create_flat_mart.sql`](DW_mart_build/03_create_flat_mart.sql) – Builds denormalized table with all dimensions joined
...
- **SQL File:** [`04_create_skills_mart.sql`](DW_mart_build/04_create_skills_mart.sql) – Builds time-series skill demand mart
...
- **SQL Files:**
  - [`05_create_priority_mart.sql`](DW_mart_build/05_create_priority_mart.sql) – Initial build of priority roles and jobs snapshot
  - [`06_update_priority_mart.sql`](DW_mart_build/06_update_priority_mart.sql) – **Incremental update using MERGE** (upsert pattern)
...
- **SQL File:** [`07_create_company_mart.sql`](DW_mart_build/07_create_company_mart.sql) – Builds company hiring trends mart (optional)
...
- **Orchestration:** Master SQL script (`build_marts.sql`) for automated pipeline execution  
